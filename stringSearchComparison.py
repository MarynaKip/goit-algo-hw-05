...
# алгоритм пошуку Кнута-Морріса-Пратта
...
pattern_1 = "Жадібний алгоритм – метод розв'язання оптимізаційних задач, заснований на тому, що процес прийняття рішення можна розбити на елементарні кроки, на кожному з яких приймається окреме рішення. Рішення, прийняте на кожному кроці, має бути оптимальним тільки на поточному кроці та повинне прийматися без врахування попередніх або наступних рішень."
...
pattern_2 = "Параметри 3 серії експериментів: кількість агентів 262144, кількість предметів 524288, кількість сесій 1048576, розмір сесії 256, максимальна кількість вподобань 2048."
pattern_3 = "Вигаданий текст, якого не існує у жодному з текстів"

t1_p1_bm = timeit.timeit(lambda: boyer_moore_search(text_1, pattern_1), number=10000)
print(f't1_p1_bm: {t1_p1_bm}')
t1_p1_kmp = timeit.timeit(lambda: kmp_search(text_1, pattern_1), number=10000)
print(f't1_p1_kmp: {t1_p1_kmp}')
t1_p1_rks = timeit.timeit(lambda: rabin_karp_search(text_1, pattern_1), number=10000)
print(f't1_p1_rks: {t1_p1_rks}')

t1_p3_bm = timeit.timeit(lambda: boyer_moore_search(text_1, pattern_3), number=10000)
print(f't1_p3_bm: {t1_p3_bm}')
t1_p3_kmp = timeit.timeit(lambda: kmp_search(text_1, pattern_3), number=10000)
print(f't1_p3_kmp: {t1_p3_kmp}')
t1_p3_rks = timeit.timeit(lambda: rabin_karp_search(text_1, pattern_3), number=10000)
print(f't1_p3_rks: {t1_p3_rks}')


t2_p2_bm = timeit.timeit(lambda: boyer_moore_search(text_2, pattern_2), number=10000)
print(f't2_p2_bm: {t2_p2_bm}')
t2_p2_kmp = timeit.timeit(lambda: kmp_search(text_2, pattern_2), number=10000)
print(f't2_p2_kmp: {t2_p2_kmp}')
t2_p2_rks = timeit.timeit(lambda: rabin_karp_search(text_2, pattern_2), number=10000)
print(f't2_p2_rks: {t2_p2_rks}')

t2_p3_bm = timeit.timeit(lambda: boyer_moore_search(text_2, pattern_3), number=10000)
print(f't2_p3_bm: {t2_p3_bm}')
t2_p3_kmp = timeit.timeit(lambda: kmp_search(text_2, pattern_3), number=10000)
print(f't2_p3_kmp: {t2_p3_kmp}')
t2_p3_rks = timeit.timeit(lambda: rabin_karp_search(text_2, pattern_3), number=10000)
print(f't2_p3_rks: {t2_p3_rks}')
